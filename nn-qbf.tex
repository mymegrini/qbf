...
\newline 
Pour résoudre une formule on utilisera deux réseaux qui s'affronteront dans un jeu. Un réseau représentera les quantificateurs universels et l'autre représentera les quantificateurs existentiels. Celui qui représentera les quantificateurs universels aura pour but de choisir des valuations pour les variables qui permettrons de rendre la formule fausse. Le réseau qui représentera les quantificateur existentiels aura pour but de choisir des valuations pour les variables qui permettrons de rendre vrai la formule. Lorsque l'algorithme sera terminé, on aura une valeur qui tendra soit vers 0 si la formule est supposé être fausse (donc le réseau universels aura gagné)  soit vers 1 pour le cas contraire (le réseau existentiels aura gagné). 
\\
...
Pour fonctionner, notre algorithme a besoin d'une formule QBF qu'on lui donnera via un fichier sous format qdimacs. Ce format, est le format standard des QBF solvers.
Un fichier de type qdimacs commence toujours par "p cnf $m$ $n$" où m est le nombre de variables et n le nombre de clauses que contient la formule. Chaque variable apparaîtra dans le fichier sous forme d'entier compris entre 1 et m. Si la variable est négative cela signifie qu'on prends sa négation. Les affectations des quantificateurs aux variables se fait au début du fichier juste aprés la ligne "p cnf m n".
On affecte un quantificateur existentiel à la varialbe $n_1$ de la manière suivante : e $n_1$ 0 \\
On affecte un quantificateur universel à la varialbe $n_1$ de la manière suivante : a $n_1$ 0 \\
On ne peut pas mettre à la suite deux affectations de quantificateurs de même type, on doit faire l'affectation en une seul ligne.
Chaque ligne qui déclare une clause se termine par 0.
Les variables qui ne sont pas rattaché à un quantifiateur sont considérées comme étant existentiels.\\ 
\textbf{Exemple de fihier en format qdimacs :} \\
\\
p cnf 4 4\\ 
e 1 0\\
a 2 0\\
...
Ce fichier représente la clause suivante : \\ 
$\exists x \forall y \exists z \forall t (x \land y) \lor (y \land \lnot x) \lor (t \land \lnot z) \lor (z \land \lnot t) $  
